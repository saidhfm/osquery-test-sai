...
// Process information cache for ancestry collection
using ProcessInfoCache = std::unordered_map<pid_t, JSON>;
...
struct ProcessInfo {
  ...
  ProcessInfo() : pid(0), ppid(0) {}
  ProcessInfo(pid_t p, pid_t pp, const std::string& path_val, const std::string& cmd_val)
    : pid(p), ppid(pp), path(path_val), cmdline(cmd_val) {}
};

// Helper function to get process attribute path
inline std::string getProcAttr(const std::string& attr, pid_t pid) {
  return "/proc/" + std::to_string(pid) + "/" + attr;
}

// Helper function to read process command line
std::string readProcCmdline(pid_t pid) {
  ...
  // Replace null delimiters with spaces
  std::replace_if(
      content.begin(),
      content.end(),
...
// Helper function to read process executable path
std::string readProcExePath(pid_t pid) {
  auto attr = getProcAttr("exe", pid);
  std::string path;
  if (!readFile(attr, path).ok()) {
    return "";
  }
  return path;
}

// Helper function to get parent PID from /proc/pid/stat
pid_t readProcParentPid(pid_t pid) {
  auto attr = getProcAttr("stat", pid);
  std::string content;
  if (!readFile(attr, content).ok()) {
    return 0;
  }
  
  auto fields = osquery::split(content, " ");
  if (fields.size() < 4) {
    return 0;
  }
  
  // PPID is the 4th field (0-indexed: 3)
  try {
    return static_cast<pid_t>(std::stoll(fields[3]));
...
bool getProcessInfo(pid_t pid, ProcessInfo& info, ProcessInfoCache& cache) {
  // Check cache first
  auto it = cache.find(pid);
  if (it != cache.end()) {
    auto& cached = it->second;
    info.pid = cached["pid"].get<pid_t>();
    info.ppid = cached["ppid"].get<pid_t>();
    info.path = cached["path"].get<std::string>();
    info.cmdline = cached["cmdline"].get<std::string>();
    return true;
  }
  
  // Read process information
  info.pid = pid;
  info.ppid = readProcParentPid(pid);
  info.path = readProcExePath(pid);
  info.cmdline = readProcCmdline(pid);
  
  // Cache successful reads
  if (info.ppid > 0) {
    JSON cached_info;
    cached_info["pid"] = info.pid;
    cached_info["ppid"] = info.ppid;
    cached_info["path"] = info.path;
    cached_info["cmdline"] = info.cmdline;
    cache[pid] = cached_info;
    return true;
  }
...
FLAG(bool,
     process_events_enable_ancestry,
     true,
     "Enable process ancestry collection for process_events table");

FLAG(uint32,
     process_events_max_ancestry_depth,
     10,
     "Maximum depth to traverse when building process ancestry chain");
...
Status AuditProcessEventSubscriber::ProcessEvents(
  ...
  */
  ...
  for (const auto& event : event_list) {
    ...
    // Build process ancestry
    row["ancestry"] = BuildProcessAncestry(static_cast<pid_t>(process_id));
...
std::string AuditProcessEventSubscriber::BuildProcessAncestry(pid_t pid) noexcept {
  if (!FLAGS_process_events_enable_ancestry) {
    return "{}";
  }
  
  try {
    auto ancestry_json = JSON::newObject();
    ProcessInfoCache cache;
    
    std::set<pid_t> visited_pids; // Prevent infinite loops
    pid_t current_pid = pid;
    uint32_t depth = 0;
    bool truncated = false;
    
    // Create the ancestors array
    auto ancestors_array = ancestry_json.getArray();
    
    // Walk up the process tree
    while (current_pid > 1 && depth < FLAGS_process_events_max_ancestry_depth) {
      // Check for cycles
      if (visited_pids.count(current_pid) > 0) {
        VLOG(1) << "Process ancestry cycle detected at PID " << current_pid;
        break;
      }
      visited_pids.insert(current_pid);
      
      ProcessInfo info;
      if (!getProcessInfo(current_pid, info, cache)) {
        VLOG(1) << "Failed to get process info for PID " << current_pid;
        break;
      }
      
      // Stop if we reach the same process (shouldn't happen with proper ppid)
      if (info.ppid == current_pid) {
        break;
      }
      
      // Only process if we have valid parent data
      if (info.ppid > 0) {
        // Create ancestor object
        auto ancestor_obj = ancestry_json.getObject();
        ancestry_json.addCopy("pid", std::to_string(info.ppid), ancestor_obj);
        
        pid_t grandparent_pid = readProcParentPid(info.ppid);
        ancestry_json.addCopy("ppid", std::to_string(grandparent_pid), ancestor_obj);
        ancestry_json.addCopy("path", readProcExePath(info.ppid), ancestor_obj);
        ancestry_json.addCopy("cmdline", readProcCmdline(info.ppid), ancestor_obj);
        ancestry_json.addCopy("depth", std::to_string(depth + 1), ancestor_obj);
        
        // Push the ancestor object to the array
        ancestry_json.push(ancestor_obj, ancestors_array);
      }
    ...
    if (depth >= FLAGS_process_events_max_ancestry_depth && current_pid > 1) {
      truncated = true;
    }
    
    // Build final ancestry object
    ancestry_json.add("ancestors", ancestors_array);
    ancestry_json.add("depth", depth);
    ancestry_json.add("truncated", truncated);
    
    std::string result;
    auto status = ancestry_json.toString(result);
    if (!status.ok()) {
      VLOG(1) << "Failed to serialize ancestry JSON: " << status.getMessage();
      return "{}";
    }
    ...
    VLOG(1) << "Exception in BuildProcessAncestry: " << e.what();
    return "{}";
  } catch (...) {
    VLOG(1) << "Unknown exception in BuildProcessAncestry";
    return "{}";
  }
}
} // namespace osquery
